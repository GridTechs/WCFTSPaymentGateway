<?php
/*
 * Main Gateway of FTSCoin using either a local daemon or the explorer
 * Authors: Serhack, cryptochangements, mosu-forge
 */

defined( 'ABSPATH' ) || exit;

require_once('class-ftscoin-service.php');

class FTSCoin_Gateway extends WC_Payment_Gateway
{
    private static $_id = 'ftscoin_gateway';
    private static $_title = 'FTSCoin Gateway';
    private static $_method_title = 'FTSCoin Gateway';
    private static $_method_description = 'FTSCoin Gateway Plug-in for WooCommerce.';
    ...
    private static $use_ftscoin_price = false;
    private static $use_ftscoin_price_decimals = FTSCOIN_GATEWAY_ATOMIC_UNITS;

    private static $ftscoin_service;
    private static $log;
    ...
    {
        return apply_filters('woocommerce_gateway_icon', '<img src="'.FTSCOIN_GATEWAY_PLUGIN_URL.'assets/images/ftscoin-icon.png"/>', $this->id);
    }

    function __construct($add_action=true)
    {
        $this->id = self::$_id;
        $this->method_title = __(self::$_method_title, 'ftscoin_gateway');
        $this->method_description = __(self::$_method_description, 'ftscoin_gateway');
        ...
        self::$address = $this->settings['ftscoin_address'];
        ...
        self::$use_ftscoin_price = $this->settings['use_ftscoin_price'] == 'yes';
        self::$use_ftscoin_price_decimals = $this->settings['use_ftscoin_price_decimals'];

        $explorer_url = FTSCOIN_GATEWAY_EXPLORER_URL;

        if($add_action)
            add_action('woocommerce_update_options_payment_gateways_'.$this->id, array($this, 'process_admin_options'));

        // Helper functions
        self::$ftscoin_service = new FTScoin_Library(self::$host, self::$port, self::$password);
        self::$log = new WC_Logger();
    }

    public function init_form_fields()
    {
        $this->form_fields = include 'admin/ftscoin-gateway-admin-settings.php';
    }

    public function validate_ftscoin_address_field($key,$address)
    {

        if (strlen($address) == 98 && substr($address, 0, 3) == 'fts')
            return $address;
        self::$_errors[] = 'FTS address is invalid';
    ...
    public function admin_options()
    {
        ...
        include FTSCOIN_GATEWAY_PLUGIN_DIR . '/templates/ftscoin-gateway/admin/settings-page.php';
    }
    ...
    public static function admin_balance_info()
    {
        ...
        $wallet_amount = self::$ftscoin_service->getBalance();
        $height = self::$ftscoin_service->getHeight();

        if (!isset($wallet_amount)) {
            self::$_errors[] = 'Cannot connect to ftscoin-wallet-rpc';
            self::$log->add('FTSCoin_Payments', '[ERROR] Cannot connect to ftscoin-wallet-rpc');
            return array(
                'height' => 'Not Available',
                'balance' => 'Not Available',
                'unlocked_balance' => 'Not Available',
            ...
            return array(
                'height' => $height,
                'balance' => self::format_ftscoin($wallet_amount['lockedAmount']).' FTS',
                'unlocked_balance' => self::format_ftscoin($wallet_amount['availableBalance']).' FTS'
            );
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        $ftscoin_amount = 1e8 * $fiat_amount / $rate;

        if(self::$discount)
            $ftscoin_amount = $ftscoin_amount - $ftscoin_amount * self::$discount / 100;

        // only use whole numbers for FTS
        $ftscoin_amount = intval($ftscoin_amount);

        $ftscoin_amount = intval($ftscoin_amount * FTSCOIN_GATEWAY_ATOMIC_UNITS_POW);

        $query = $wpdb->prepare("INSERT INTO $table_name (order_id, payment_id, currency, rate, amount) VALUES (%d, %s, %s, %d, %d)", array($order_id, $payment_id, $currency, $rate, $ftscoin_amount));
        $wpdb->query($query);

        $order->update_status('on-hold', __('Awaiting offline payment', 'ftscoin_gateway'));
    ...
    public static function do_update_event()
    {
        ...
        // Get Live Price
        $FTS_link = 'https://cratex.io/api/v1/get_markets.php?market=FTS/BTC';
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_RETURNTRANSFER => 1,
            CURLOPT_URL => $FTS_link,
        ));
        $resp = curl_exec($curl);
        curl_close($curl);
        ...
        $height = self::$ftscoin_service->getHeight();
        set_transient('ftscoin_gateway_network_height', $height);

        // Get pending payments
        $table_name_1 = $wpdb->prefix.'ftscoin_gateway_quotes';
        $table_name_2 = $wpdb->prefix.'ftscoin_gateway_quotes_txids';
        ...
        foreach($pending_payments as $pending) {
            ...
            $amount_ftscoin = $quote->amount_total;
            ...
            $paid = $amount_paid > $amount_ftscoin - FTSCOIN_GATEWAY_ATOMIC_UNIT_THRESHOLD;
            ...
            if($paid && $confirmed) {
                self::$log->add('FTSCoin_Payments', "[SUCCESS] Payment has been confirmed for order id $order_id and payment id $payment_id");
                ...
                if(self::is_virtual_in_cart($order_id) == true){
                    $order->update_status('completed', __('Payment has been received.', 'ftscoin_gateway'));
                } else {
                    $order->update_status('processing', __('Payment has been received.', 'ftscoin_gateway'));
                }

            } else if($paid) {
                self::$log->add('FTSCoin_Payments', "[SUCCESS] Payment has been received for order id $order_id and payment id $payment_id");
                ...
                if($order_age_seconds > self::$valid_time) {
                    self::$log->add('FTSCoin_Payments', "[FAILED] Payment has expired for order id $order_id and payment id $payment_id");
                    $query = $wpdb->prepare("UPDATE $table_name_1 SET pending=0 WHERE payment_id=%s", array($payment_id));
                    $wpdb->query($query);

                    unset(self::$payment_details[$order_id]);

                    $order->update_status('cancelled', __('Payment has expired.', 'ftscoin_gateway'));
                }
            }
        }
    }
    ...
    {
        $txs = array();
        $payments = self::$ftscoin_service->get_all_payments($payment_id);
        ...
        if (count($details)) {
            ...
            foreach($details as $tx) {
                ...
                $txs[] = array(
                    ...
                    'amount_formatted' => self::format_ftscoin($tx->amount_paid)
                );
            ...
            if(count($heights) && !in_array(0, $heights)) {
                $height = get_transient('ftscoin_gateway_network_height');
                $highest_block = max($heights);
            ...
            $time_to_confirm = self::format_seconds_to_time($blocks_to_confirm * FTSCOIN_GATEWAY_DIFFICULTY_TARGET);
            ...
            $array_integrated_address = self::$ftscoin_service->make_integrated_address($address, $payment_id);
            if (isset($array_integrated_address['integratedAddress'])) {
                $integrated_addr = $array_integrated_address['integratedAddress'];
            } else {
                self::$log->add('FTSCoin_Gateway', '[ERROR] Unable get integrated address');
            ...
            $qrcode_uri = 'ftscoin:'.$address.'?tx_amount='.$amount_due.'&tx_payment_id='.$payment_id;
            ...
            $payment_details = array(
                ...
                'amount_total_formatted' => self::format_ftscoin($amount_total),
                'amount_paid_formatted' => self::format_ftscoin($amount_paid),
                'amount_due_formatted' => self::format_ftscoin($amount_due),
                'status' => $status,
    ...
    public static function get_payment_details_ajax() {
        ...
        if($order->get_payment_method() != self::$_id)
            self::ajax_output(array('error' => '[ERROR] Order not paid for with FTSCoin'));
    ...
    public static function admin_order_page($post)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include FTSCOIN_GATEWAY_PLUGIN_DIR . '/templates/ftscoin-gateway/admin/order-history-error-page.php';
            return;
        }
        include FTSCOIN_GATEWAY_PLUGIN_DIR . '/templates/ftscoin-gateway/admin/order-history-page.php';
    }
    ...
    public static function customer_order_page($order)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include FTSCOIN_GATEWAY_PLUGIN_DIR . '/templates/ftscoin-gateway/customer/order-error-page.php';
            return;
        }
        $show_qr = self::$show_qr;
        $details_json = json_encode($details);
        $ajax_url = WC_AJAX::get_endpoint('ftscoin_gateway_payment_details');
        include FTSCOIN_GATEWAY_PLUGIN_DIR . '/templates/ftscoin-gateway/customer/order-page.php';
    }
    ...
    public static function customer_order_email($order)
    {
        ...
        $details = self::get_payment_details($order_id);
        if(!is_array($details)) {
            include FTSCOIN_GATEWAY_PLUGIN_DIR . '/templates/ftscoin-gateway/customer/order-email-error-block.php';
            return;
        }
        include FTSCOIN_GATEWAY_PLUGIN_DIR . '/templates/ftscoin-gateway/customer/order-email-block.php';
    }
    ...
    public static function use_ftscoin_price()
    {
        return self::$use_ftscoin_price;
    }
    ...
    {
        $rate = self::get_live_rate($currency);
        $ftscoin_amount = intval(FTSCOIN_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / FTSCOIN_GATEWAY_ATOMIC_UNITS_POW;
        $ftscoin_amount_formatted = number_format($ftscoin_amount, self::$use_ftscoin_price_decimals);

        return <<<HTML
            <span class="woocommerce-Price-amount amount" data-price="$price" data-currency="$currency"
        data-rate="$rate" data-rate-type="live">
            $ftscoin_amount_formatted
            <span class="woocommerce-Price-currencySymbol">FTS</span>
        </span>
...
HTML;
    ...
    public static function format_ftscoin($atomic_units) {
        return sprintf(FTSCOIN_GATEWAY_ATOMIC_UNITS_SPRINTF, $atomic_units / FTSCOIN_GATEWAY_ATOMIC_UNITS_POW);
    }
